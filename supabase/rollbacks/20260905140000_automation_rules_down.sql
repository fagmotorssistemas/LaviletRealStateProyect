-- Revierte 20260905140000_automation_rules.sql
-- Restaura apply_lead_events y handoff_lead con cortes/SLA hardcodeados (25/60 y 120).
...
ALTER TABLE public.project_automation_config
  DROP COLUMN IF EXISTS sla_response_minutes,
  DROP COLUMN IF EXISTS temperature_warm_min,
  DROP COLUMN IF EXISTS temperature_hot_min;
...
AS $function$
declare
  v_event text; v_rule public.lead_scoring_rules; v_key text;
  v_old_temp text; v_new_temp text; v_score integer; v_handoff_reason text;
begin
  select l.temperature into v_old_temp from public.leads l where l.id=p_lead_id for update;
  ...
  v_new_temp := case when v_score>=60 then 'caliente' when v_score>=25 then 'tibio' else 'frio' end;

  update public.leads set temperature=v_new_temp, temperature_score=v_score,
    temperature_updated_at=now(), updated_at=now() where id=p_lead_id;
  ...
  if v_old_temp is distinct from v_new_temp then
    ...
    values(p_lead_id,v_old_temp,v_new_temp,v_score,
      coalesce((select string_agg(r.reason,', ') from public.lead_scoring_rules r
                where r.event_type in (select jsonb_array_elements_text(coalesce(p_events,'[]'::jsonb)))),
               'recalculo por eventos'),now());
  end if;
  ...
  elsif coalesce(p_events,'[]'::jsonb) ?| array['asked_price','asked_financing','requested_visit'] or v_new_temp='caliente' then
  ...
  select case
    ...
    when v_new_temp='caliente' then 'alcanzo 60 puntos o mas'
    else null end into v_handoff_reason;
...
begin
  ...
  if v_open then
    ...
    v_due := public.add_business_minutes(v_project, now(), 120);
...
SET search_path TO public
AS $function$
declare c public.project_automation_config; local_at timestamp; d jsonb;
begin
 select * into c from public.project_automation_config where project_id=p_project_id and is_active;
 if not found then raise exception 'Falta project_automation_config para el proyecto %',p_project_id; end if;
