-- INSTALACIÓN: ejecutar una vez en SQL Editor con rol postgres. No borra conversaciones al instalar.
-- Después usar lv_reset_lavilet_pablo_lead() o lv_reset_lavilet_nataly_lead().
BEGIN;

-- Reinicio manual limitado al contacto de prueba autorizado. No se ejecuta al instalar la función.
CREATE OR REPLACE FUNCTION public.lv_reset_lavilet_test_contact(p_kommo_id integer)
...
BEGIN
 -- Tres identidades autorizadas; nunca aceptar un UUID arbitrario o buscar por nombre.
 CASE p_kommo_id
  WHEN 2710090 THEN lid:='52fa6e93-4bd4-42ad-963a-666e9c7902a7'; expected_phone:='593987110032';
  WHEN 3928256 THEN lid:='bf32833f-fd65-461e-9e0e-31844e4d6814'; expected_phone:='593939731833';
  WHEN 3577404 THEN lid:='e49607f2-ba8d-4a2b-a607-85617072800c'; expected_phone:='593987077120';
  ELSE RAISE EXCEPTION 'Solo se permite reiniciar los contactos de prueba autorizados de Carlos, Nataly y Pablo.';
 END CASE;
 ...
 OR l.kommo_id IS DISTINCT FROM p_kommo_id
 ...
 OR EXISTS(SELECT 1 FROM public.lv_integration_events WHERE tenant_id=l.tenant_id AND project_id=l.project_id
  AND (payload->>'kommoId'=l.kommo_id::text OR payload->>'leadId'=lid::text) AND status='processing') THEN
 ...
 WHERE tenant_id=l.tenant_id AND project_id=l.project_id AND (
...
RETURNS jsonb LANGUAGE sql SECURITY INVOKER SET search_path='' AS $fn$
 SELECT public.lv_reset_lavilet_test_contact(2710090);
$fn$;

CREATE OR REPLACE FUNCTION public.lv_reset_lavilet_nataly_lead()
RETURNS jsonb LANGUAGE sql SECURITY INVOKER SET search_path='' AS $fn$
 SELECT public.lv_reset_lavilet_test_contact(3928256);
$fn$;

CREATE OR REPLACE FUNCTION public.lv_reset_lavilet_pablo_lead()
RETURNS jsonb LANGUAGE sql SECURITY INVOKER SET search_path='' AS $fn$
 SELECT public.lv_reset_lavilet_test_contact(3577404);
$fn$;

-- Solo SQL Editor/administración: no exponer reinicios destructivos mediante la API.
REVOKE ALL ON FUNCTION public.lv_reset_lavilet_test_contact(integer) FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.lv_reset_lavilet_test_lead() FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.lv_reset_lavilet_nataly_lead() FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.lv_reset_lavilet_pablo_lead() FROM PUBLIC,anon,authenticated,service_role;
COMMENT ON FUNCTION public.lv_reset_lavilet_test_contact(integer) IS 'Reinicio privado con respaldo, limitado a las identidades verificadas de Carlos, Nataly y Pablo en La Vilet. No envía mensajes ni cambia Kommo.';
COMMENT ON FUNCTION public.lv_reset_lavilet_test_lead() IS 'Reinicia exclusivamente a Carlos, Kommo 2710090. Conserva identidad, respalda datos y no envía mensajes.';
COMMENT ON FUNCTION public.lv_reset_lavilet_nataly_lead() IS 'Reinicia exclusivamente a Nataly Caballero, Kommo 3928256. Conserva identidad, respalda datos y no envía mensajes.';
COMMENT ON FUNCTION public.lv_reset_lavilet_pablo_lead() IS 'Reinicia exclusivamente a Pablo Andrés, Kommo 3577404. Conserva identidad, respalda datos y no envía mensajes.';
COMMIT;
