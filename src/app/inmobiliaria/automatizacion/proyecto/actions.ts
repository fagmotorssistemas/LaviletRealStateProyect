...
export async function saveProjectReadiness(projectId:string,value:unknown,expectedUpdatedAt:string) {
  ...
  const current=validateReadiness(value)
  if(project.updated_at!==expectedUpdatedAt)throw Error('La configuración cambió. Recargue antes de guardar para conservar los cambios de otros controles.')
  ...
  const result=await supabase.from('projects').update({policies_json:next,updated_at:now}).eq('id',projectId).eq('tenant_id',project.tenant_id).eq('updated_at',expectedUpdatedAt).select('id')
  if(result.error)throw Error('No se pudo guardar la configuración')
  if(result.data?.length!==1)throw Error('La configuración cambió. Recargue e intente de nuevo.')
  return {value:current,updatedAt:now,configured:true}
}
