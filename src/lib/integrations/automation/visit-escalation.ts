...
/** A declined advisor proposal must not become a new automatic round of questions. */
export function declinesAllVisitAlternatives(current: string, proposal: Row | null, intent?: string) {
  ...
  return rejection || (intent === 'reject' && /^(?:no|no gracias|ninguno|ninguna|no puedo|no me sirve|no me viene bien)$/.test(value))
}
...
export function visitCoordinationSummary(current: string, proposal: Row, history: unknown) {
  ...
  return [
    'Coordinación urgente de visita. El cliente rechazó las alternativas ofrecidas y no indicó otra fecha u hora.',
    'Acción pendiente: llamar al cliente para acordar una fecha; verificar disponibilidad y registrar la confirmación. El bot está pausado.',
