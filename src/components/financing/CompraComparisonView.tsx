...
function Accordion({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[#e4ddd3]">
      ...
      {open ? <div className="space-y-4 bg-[#f4f1eb] px-4 py-5">{t(children)}</div> : null}
    </div>
  )
}
    ...
    <div className="space-y-5">
      ...
      >
        <h3 className="text-lg font-semibold leading-snug sm:text-[22px]">
          {t(unitTitle)} · {t(isCash ? 'Compra sin crédito' : 'Compra con financiamiento')}
        </h3>
        <p className="mt-2 text-[13px] leading-relaxed text-white/95 sm:text-sm">{t(heroSubtitle)}</p>
      </div>

      {/* Controles de horizonte */}
      <div className="grid grid-cols-2 gap-3">
        <label className="block space-y-1 text-[11px]">
          <span className="text-[#6b645c]">{t("Horizonte (años)")}</span>
          <input
            type="number"
            min={1}
            max={40}
        ...
        <label className="block space-y-1 text-[11px]">
          <span className="text-[#6b645c]">{t("Apreciación %/año")}</span>
          <input
            type="number"
            step={0.1}
      ...
      <div className="space-y-3">
        <Accordion
          ...
          title={t("¿Cómo el alquiler ayuda con la cuota?")}
        >
          <div className="space-y-3">
            <Row label={t("Alquiler bruto")} value={formatMoneyExact(grossMo)} />
            <Row
              label={t(`Vacancia (${formatPercent(preview.vacancyRate * 100, 0)})`)}
              value={`−${formatMoneyExact(vacancyMo)}`}
            />
            <Row label={t("Gastos de la propiedad")} value={`−${formatMoneyExact(opsMo)}`} />
            {adminTaxMo > 0 ? (
              <Row
                label={t([
                  mgmtMo > 0
          ...
          {!isCash && payment > 0 && rentPctLabel != null && buyerPctLabel != null ? (
            <div className="space-y-2">
              <p className="text-[12px] text-[#6b645c]">
                {t(" Cobertura de la cuota (")}{t(formatMoneyExact(payment))})
              </p>
              <div className="flex h-6 overflow-hidden rounded">
                {rentPctLabel > 0 ? (
                  ...
                  >
                    {t(rentPctLabel >= 18 ? `${rentPctLabel}% Alquiler` : null)}
                  </div>
                ) : null}
                {buyerPctLabel > 0 ? (
                  <div
                  ...
                  >
                    {t(buyerPctLabel >= 18 ? `${buyerPctLabel}% Tú` : null)}
                  </div>
                ) : null}
              </div>
              <p className="rounded-lg bg-emerald-50 px-3 py-3 text-[13px] font-medium text-emerald-800">
                {t(" El alquiler neto cubre ")}{t(formatMoneyExact(bar.rentApplied))} (
                {t(formatPercent(bar.rentPct ?? 0))}{t(") de la cuota. Tú completas")}{t(' ')}
                {t(formatMoneyExact(monthlyFlow < 0 ? Math.abs(monthlyFlow) : 0))}.
              </p>
              {bar.surplusOverPayment > 0 ? (
                <p className="text-[12px] text-[#6b645c]">
                  {t(" Excedente tras la cuota: ")}{t(formatMoneyExact(bar.surplusOverPayment))}{t("/mes ")}</p>
              ) : null}
            </div>
          ) : (
          ...
          {(mgmtMo > 0 || taxMo > 0) && (
            <p className="text-[11px] text-[#8a8176]">
              {t(" Admin e IR son supuestos del escenario; no son tasas legales universales. ")}</p>
          )}
        </Accordion>
        ...
        <Accordion
          ...
          openId={openAccordion}
          onToggle={toggleAccordion}
          title={t(`Tu deuda y patrimonio (${horizonYears} años)`)}
        >
          {!isCash ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                {(
                  ...
                  <div key={label} className="rounded-md bg-white/10 px-3 py-3">
                    <p className="text-[11px] tracking-[0.08em] text-white/70 uppercase">{t(label)}</p>
                    <p className="mt-1 text-lg font-bold tabular-nums text-white">{t(value)}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-lg bg-[#1a2744] p-4 text-sm text-white/90">
              {t(" Compra al contado: no hay deuda bancaria. Valor estimado de la propiedad:")}{t(' ')}
              <strong className="text-white">{t(formatMoney(wealth.futurePropertyValue))}</strong>.
            </div>
          )}

          <div className="space-y-2 rounded-lg border border-[#e4ddd3] bg-[#fcfbf9] p-4">
            <div className="flex items-center justify-between gap-3">
              <span className="text-[12px] text-[#6b645c]">{t("Tu patrimonio neto")}</span>
              <span className="text-sm font-bold tabular-nums text-[#1f1a14]">
                {t(formatMoney(wealth.endingEquity))}
              </span>
            </div>
            <p className="text-[12px] text-[#6b645c]">
              {t(" Propiedad (")}{t(formatMoney(wealth.futurePropertyValue))})
              {t(!isCash ? ` − Deuda (${formatMoney(wealth.remainingDebt)})` : '')}
              {t(wealth.saleCostsIncluded ? ` − Salida (${formatMoney(wealth.saleCosts)})` : '')} {t(" = patrimonio. No es la ganancia: aún incluye lo que aportaste. ")}</p>
          </div>

          {!isCash ? (
            <div className="rounded border-l-[3px] border-[#c4a574] bg-[#faf3e8] px-3 py-3 text-[12px] text-[#7a5c2e]">
              {t(" La deuda no desaparece sola. Con cada cuota pagas intereses y capital que reduce el préstamo. Al final del plazo (")}{t(preview.financingYears)} {t(" años), si se cumplen las cuotas, la deuda programada llega a cero. ")}</div>
          ) : null}
        </Accordion>
        ...
        <Accordion
          ...
          openId={openAccordion}
          onToggle={toggleAccordion}
          title={t("¿Cómo se calcula todo esto?")}
        >
          <div className="space-y-3">
            <Row
              label={t("Ganancia total")}
              value="Patrimonio + excedentes − capital aportado"
              tone="muted"
            />
            <Row label={t("Retorno acumulado")} value="(Ganancia ÷ capital aportado) × 100" tone="muted" />
            <Row
              label={t(`Capital aportado (${horizonYears} años)`)}
              value={`${formatMoney(wealth.initialOutlay)} + ${formatMoney(wealth.cumulativeTopUps)} = ${formatMoney(wealth.totalCashInvested)}`}
            />
            <Row
              label={t("Amortización")}
              value="Cada cuota paga intereses y reduce el préstamo"
              tone="muted"
            />
          </div>
          <div className="rounded border-l-[3px] border-[#c4a574] bg-[#faf3e8] px-3 py-3 text-[12px] text-[#7a5c2e]">
            {t(" Estimaciones educativas. Alquiler ")}{t(formatMoneyExact(grossMo))}{t(", tasa")}{t(' ')}
            {t(Number(preview.interestRate).toFixed(2))}{t("%, plazo ")}{t(preview.financingYears || '—')} {t(" años, apreciación ")}{t(formatPercent(appreciationRateAnnual * 100, 1))}{t("/año, ocupación")}{t(' ')}
            {t(formatPercent((1 - preview.vacancyRate) * 100, 0))}{t(". No constituyen asesoría financiera. ")}</div>
        </Accordion>
      </div>
      ...
      <div className="space-y-3 pt-2">
        <h4 className="text-base font-semibold text-[#1f1a14]">
          {t(" Compara: Contado vs Financiamiento ")}</h4>
        {(() => {
        ...
        <div className="rounded border-l-[3px] border-[#c4a574] bg-[#faf3e8] px-3 py-3 text-[12px] text-[#7a5c2e]">
          {!isCash ? (
            <>
              {t(" Con entrada del ")}{t(formatPercent(downPct, 0))}{t(", desembolsas un ")}{t(formatPercent(financedPct, 0))}{t(' ')}
              {t(" menos del precio al inicio y asumes pagos mensuales apoyándote en el alquiler neto. Los calendarios de aportes son distintos; un retorno acumulado mayor no demuestra por sí solo que una opción sea mejor. ")}</>
          ) : (
            <>
              {t(" Contado: mayor desembolso inicial y sin cuotas. Financiamiento: menor desembolso inicial y compromiso mensual. Un retorno acumulado mayor no demuestra por sí solo qué opción conviene. ")}</>
          )}
        </div>
      </div>
      ...
      {onRequestInfo ? (
        ...
        >
          {t(" Quiero más información ")}</button>
      ) : null}

      {/* 6. DISCLAIMER */}
      <p className="text-center text-[11px] leading-relaxed text-[#8a8176]">
        {t(" Estos cálculos son estimaciones educativas. No constituyen asesoría financiera. Consulta con un asesor antes de decidir. ")}</p>
    </div>
  )
}
