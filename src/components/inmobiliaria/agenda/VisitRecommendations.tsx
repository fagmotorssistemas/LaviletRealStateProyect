...
export function VisitRecommendations({ options, loading, error, disabled, selectedStart, selectedStarts, onSelect, onReload }: {
  ...
  return <section aria-label="Horarios recomendados" className="rounded-2xl border border-[#dfe5d5] bg-[#f6f8f1] p-4">
    ...
    <p className="mt-1 text-xs leading-relaxed text-[#858e78]">{selectedStarts ? 'Selecciona hasta tres alternativas. Revisa el mensaje antes de enviarlo al cliente.' : 'Según la agenda del asesor. Elige uno y revisa la propuesta antes de enviarla.'}</p>
      ...
      : error ? <p role="alert" className="mt-3 text-sm text-[#966543]">{error}</p>
        : !options?.slots.length ? <p className="mt-3 text-sm text-[#7e896d]">No encontramos espacios disponibles en estas fechas. Prueba otro día o solicita una reasignación.</p>
          : <div className="mt-3 grid max-h-60 gap-2 overflow-y-auto">{options.slots.map((slot, index) => {
            const selected = (selectedStarts ?? [selectedStart]).some(start => Date.parse(start) === Date.parse(slot.start_time))
            const atLimit = Boolean(selectedStarts && selectedStarts.length >= 3 && !selected)
            return <button type="button" key={slot.start_time} disabled={disabled || atLimit} aria-pressed={selected} onClick={() => onSelect(slot)} className={`flex w-full min-w-0 flex-col items-start justify-between gap-2 rounded-xl border px-3 py-3 text-left transition sm:flex-row sm:items-center sm:gap-3 focus-visible:outline-2 focus-visible:outline-[#787D62] disabled:opacity-45 ${selected ? 'border-[#87976e] bg-white ring-1 ring-[#87976e]/20' : 'border-[#e2e7d9] bg-white/70 hover:border-[#aebc98] hover:bg-white'}`}>
              ...
              <span className="flex items-center gap-1 text-xs font-semibold text-[#73815e] sm:shrink-0">{selected ? <><Check size={14} />Seleccionado</> : selectedStarts ? 'Seleccionar' : index === 0 ? 'Aceptar recomendación' : 'Usar horario'}</span>
            </button>
          })}</div>}
  </section>
}
